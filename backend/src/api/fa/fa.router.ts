...
router.post('/generar', async (req, res, next) => {
  try {
    ...
    const ws = wb.addWorksheet('Estructura');

    ws.addRow(['INEGI - ESTRUCTURA']);
    ...
    res.setHeader('Content-Disposition', `attachment; filename="Estructura_${now.toISOString().slice(0,10)}.xlsx"`);
    return res.send(buf);
  } catch (err) {
    next(err);
  }
